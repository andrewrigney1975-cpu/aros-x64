...
## Status: Phase 20 — Workbench window manager polish and a windowed Terminal app
...
### Phase 20 — Workbench window manager polish and a windowed Terminal app
- **Click-to-focus + real z-order** (`kernel/kernel.asm`) — clicking any
  window's title bar or content area brings it to the front of the
  compositor's z-order list (`basix_zorder_bring_front`) and marks it the
  scheduler's keyboard-focus target, generalized into `wm_tick`'s existing
  click-scan instead of being special-cased per window. The focused
  window's scrollbar track, arrows, and resize-widget backgrounds are
  filled with the same blue as an active title bar (their glyphs drawn on
  top afterward so they stay visible against either color); every other
  window's chrome reverts to grey. `wm_tick`/the compositor were already
  the sole owners of drag/close/redraw for any `LAUNCH`ed window — this
  phase adds focus/ordering to that same shared machinery rather than
  building a second one.
- **Scrollbar/resize chrome polish** — horizontal and vertical scroll
  arrows unified to the same size (previously visibly inconsistent), a
  title-bar layout bug fixed (a rendering "notch" at the top-right corner
  that pushed the close gadget off the true edge), the close gadget and
  window title both nudged a few pixels to stay visually aligned with the
  scroll arrows, and white lines added to mark out the scrollbar "track"
  the thumb moves within.
- **Desktop RAM usage stat** (`kernel/kernel.asm`, `examples/workbench.bas`)
  — the desktop's top status bar now shows `<used>KB used of <total>KB
  total` next to the OS title, sourced from the kernel's own physical
  memory allocator bookkeeping rather than a static/faked number.
- **Auto-boot to GUI** (`kernel/kernel.asm`) — `shell_main` now invokes the
  exact same code path the interactive `gui` shell command already used
  (`shell_dispatch` against the existing `shell_str_gui` constant) once,
  right after the boot self-test sequence completes, instead of always
  dropping straight into the text prompt. The text shell is unchanged and
  still reachable — it's just no longer the first thing shown, and
  `workbench.bas` exiting still falls through into it exactly as before.
- **`examples/terminal.bas`** — a new windowed program, `LAUNCH`ed like
  editor.bas/viewer.bas (real kernel-drawn chrome, not a hand-rolled
  window), providing a small BASIX64-native command shell distinct from
  the boot-time text shell: `RUN <file>` (via `LAUNCH`), `DIR`, `CD <name>`
  / `CD ..`, `CLS`/`CLEAR`, `HELP`, `EXIT`. Built entirely from existing
  BASIX64 primitives (`KEYHIT`/`GETKEY`/`DRAWCHAR`/`WINCLOSE`/`LAUNCH`/
  `DIROPEN`/`DIRNEXT`/`DIRISDIR`/`DIRNAMELEN`/`DIRNAMECHAR`/`DIRCD`/
  `DIRUP`) — a deliberately smaller command set than the real boot shell,
  not a windowed port of it (a much larger, riskier refactor that was
  considered and explicitly deferred in favor of this).
- **Terminal desktop integration** (`examples/filebrowser.bas`,
  `scripts/gen_icons.py`, `assets/icon_terminal.png`) — a new custom icon
  (black terminal-screen glyph with a green `>_` prompt, generated by a new
  `gen_file_terminal` in the existing icon-generation script) and a new
  entry classification in `filebrowser.bas`'s directory scan that matches
  `TERMINAL.BAS` by exact filename *after* the generic `.BAS` check runs
  (so it overrides that default), making double-clicking it in the root
  folder launch the app directly instead of opening it in the text editor
  like an ordinary `.bas` file.
- One real BASIX64 authoring bug found and fixed while building this
  phase, in both `terminal.bas` and a copy-pasted occurrence in
  `filebrowser.bas`: a single-line `IF cond THEN stmt` form is
  self-contained and must **not** be followed by an `ENDIF` — only the
  multi-line `IF cond THEN` (body on following lines) form needs one.
  Writing two stacked single-line `IF`s each followed by a stray `ENDIF`
  (to uppercase-fold a character) silently produced a full "BASIX64
  compile error" with no clearer diagnostic; root-caused via a bisection
  sequence of progressively larger test programs run headlessly against
  the compiler.
